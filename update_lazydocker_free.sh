#!/bin/bash
# Stage the FREE-lane (apt-free) lazydocker artifacts under free/.
...
# update_lazydocker.sh on each release and from free_refresh.sh daily — the
# window advances with time even when upstream is quiet.
set -euo pipefail
REPO=lazydocker-debian
PACKAGE_NAME=lazydocker
# Same list as update_lazydocker.sh, so both lanes serve the same set. The
# Ubuntu releases only carry amd64/arm64/armhf; the rest 404 and are skipped.
